  ...
  df_scans_average$Met <- c(0.299,0.339,0.188,0.155)
    df_scans_average$Thr <- c(0.616,0.764,0.819,0.484)
      df_scans_average$Tyr <- c(0.396,0.499,0.495,0.315) ## Tyrosine
        ...
        df_scans_average$TotalProt <-c(rep(0.58515),rep(0.75595),rep(0.6432),rep(0.4319))
...
df_sc$TotalProt<-c(rep(0.58515,460),rep(0.75595,423),rep(0.6432,418),rep(0.4319,416))
df_scans_average$TotalProt <-c(rep(0.58515),rep(0.75595),rep(0.6432),rep(0.4319))
 
#Preotein Ramanone paper: 853, 1002, 1242, 1308cm−1, etc / 606, 992, 117, 11586, 2850, 3063 https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3251995/
...
plot_TotalProtein1 <- ggplot(data= df_scans_average, aes(x = X1015X1026, y = TotalProt))+
  ...
  geom_smooth(method = lm, color = "goldenrod3", fill = "goldenrod3")+
  labs(title= "Aromatic peaks", x=" ", y=" ")+
  geom_errorbar(aes(ymin= TotalProt-c(0.042,0.047,0.026,0.022), ymax= TotalProt+c(0.042,0.047,0.026,0.022)), size=0.3, width = 0.00007)+
  geom_errorbarh (aes(xmin = X1015X1026-sd_x , xmax=X1015X1026+sd_x ), size=0.3, height = 0.03)+
  geom_text(label = c( " ", " ",  " ",expression(paste("R"^"2", "= 0.92"))), vjust= -6)+
  scale_x_continuous(labels=fancy_scientific)


##Peaks I found
colnames(df_scans_average)[40]<- "X563"
colnames(df_scans_average)[41]<- "X567"
colnames(df_scans_average)[203]<- "X1188"
colnames(df_scans_average)[213]<- "X1224"
colnames(df_scans_average)[292]<- "X1502"
colnames(df_scans_average)[293]<- "X1505"

df_scans_average$X563X567 <- df_scans_average$X563 + df_scans_average$X567 + df_scans_average$X1188 + df_scans_average$X1224+
  df_scans_average$X1502 #2 prot peaks

sp_TotalProtein <- ggscatter(df_scans_average, x = "X563X567", y ="TotalProt",
   add = "reg.line",  # Add regressin line
   add.params = list(color = "goldenrod3", fill = "goldenrod3"), # Customize reg. line
   conf.int = TRUE # Add confidence interval
   )
sp_TotalProtein<-sp_TotalProtein+stat_cor( aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")))+
  labs(title= "Best correlation", x=" ", y=" ")

#Totalprot <- ggarrange(sp_TotalProtein, sp_TotalProtein1, ncol = 2)

## calculate std of pooled groups
sd_C <- sd(rbind (subset(df_sc$`563`,cell.name=="c.necator"),subset(df_sc$`567`,cell.name=="c.necator"), subset(df_sc$`1188`,cell.name=="c.necator"),subset(df_sc$`1224`,cell.name=="c.necator"),subset(df_sc$`1502`,cell.name=="c.necator")))

sd_M <- sd(rbind (subset(df_sc$`563`,cell.name=="m.extorame"),subset(df_sc$`567`,cell.name=="m.extorame"),
                  subset(df_sc$`1188`,cell.name=="m.extorame"),subset(df_sc$`1224`,cell.name=="m.extorame"),subset(df_sc$`1502`,cell.name=="m.extorame")))

sd_P <- sd(rbind (subset(df_sc$`563`,cell.name=="p.pastons"),subset(df_sc$`567`,cell.name=="p.pastons"),
                  subset(df_sc$`1188`,cell.name=="p.pastons"),subset(df_sc$`1224`,cell.name=="p.pastons"),subset(df_sc$`1502`,cell.name=="p.pastons")))

sd_Y <- sd(rbind (subset(df_sc$`563`,cell.name=="y.lipolytica"),subset(df_sc$`567`,cell.name=="y.lipolytica"),
                  subset(df_sc$`1188`,cell.name=="y.lipolytica"),subset(df_sc$`1224`,cell.name=="y.lipolytica"),subset(df_sc$`1502`,cell.name=="y.lipolytica")))

sd_x <- as.vector(cbind(sd_C, sd_M, sd_P, sd_Y))


plot_TotalProtein <- ggplot(data= df_scans_average, aes(x = X563X567, y = TotalProt))+
  geom_point(size=2.5)+
  theme_classic()+
  geom_smooth(method = lm, color = "goldenrod3", fill = "goldenrod3")+
  labs(title= "Best calibration", x=" ", y=" ")+
  geom_errorbar(aes(ymin= TotalProt-c(0.042,0.047,0.026,0.022), ymax= TotalProt+c(0.042,0.047,0.026,0.022)), size=0.3, width = 0.00005)+
  geom_errorbarh (aes(xmin = X563X567-sd_x , xmax= X563X567+sd_x ), size=0.3, height = 0.03)+
  geom_text(label = c( " ", " ",  " ",expression(paste("R"^"2", "= 0.9"))), vjust= -6)+
  scale_x_continuous(labels=fancy_scientific)

Totalprot <- ggarrange(plot_TotalProtein, plot_TotalProtein1, ncol = 2)

All_together <- ggpubr::ggarrange(All_correlations ,Totalprot, labels = c("A", "B"), nrow = 2) 
